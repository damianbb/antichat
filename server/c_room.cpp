...
void c_room::add_user (const c_user &user) { users.push_back(user); }

void c_room::remove_user (const c_user &user) { users.remove(user); }
...
list<c_user> &c_room::get_users () { return users; }
...
c_user &c_room::get_user_by_nickname (const string &nickname) {
	for (c_user &user : users) {
